...
if (args[1] == 'run') {
  ...
  for (i in (id+TASKID):(id+TASKID+STEPSIZE-1)) {
    ...
    print(sessionInfo())
    # load data
    # for(i in seq_len(nrow(parm_red))){
      # cat(i, "\n")
    ...
    do.one <- function(i){
      ...
      parm <- expand.grid(seed = 1:bigB,
                          ...
                          wrapper = wrappers,
                          stringsAsFactors = FALSE)
      parm_red <- parm[parm$K == parm$K[1] & parm$wrapper == "randomforest_wrapper",]

      load(paste0("~/cvtmleauc/scratch/dataList_",
                  "n=",parm_red$n[i],
                  "_seed=",parm_red$seed[i], 
                  ".RData"))
    ...
    # fit on full data
    fit_full <- do.call(randomforest_wrapper, args = list(train = list(X = dat$X, Y = dat$Y), 
    ...
    for(k in c(5, 10, 20, 40)){
      tryCatch({
        load(paste0("~/cvtmleauc/out/outtn_",
                          "n=", parm_red$n[i],
                          "_seed=",parm_red$seed[i],
                          "_K=",k,
                          "_wrapper=randomforest_wrapper",
                          ".RData"))

        out[length(out)] <- true_parameter
